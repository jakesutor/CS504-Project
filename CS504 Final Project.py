...
logit_model=sm.Logit(y,x_scaled_filtered)
result=logit_model.fit()
print(result.summary2())

#split the data into training and testing data
x_train, x_test, y_train, y_test = train_test_split(x_scaled_filtered,y,
...
x_train, x_test, y_train, y_test = train_test_split(x_scaled_filtered, y, random_state = 42, stratify = y)
...
best_features = pd.DataFrame(rf_best.feature_importances_.reshape(1,11), columns = selected_features).T
...
plt.figure(figsize=(15,10))
plt.scatter(teamsFiltered2.PER,teamsFiltered2.WS,c=teamsFiltered2.ALL_STAR)
plt.title('PER Against WS', fontsize=20)
plt.xlabel('PER', fontsize=16)
plt.ylabel('WS', fontsize=16)

# Again, clear distinction between All Stars' PER and others
plt.figure(figsize=(15,10))
plt.scatter(teamsFiltered2.PER,teamsFiltered2.MP,c=teamsFiltered2.ALL_STAR)
plt.title('PER Against Minutes Played', fontsize=20)
plt.xlabel('PER', fontsize=16)
plt.ylabel('MP', fontsize=16)

# PTS scored also favors the All Stars, but there are more non-All Stars close to All-Star level
plt.figure(figsize=(15,10))
plt.scatter(teamsFiltered2.PTS,teamsFiltered2.MP,c=teamsFiltered2.ALL_STAR)
plt.title('PTS Against Minutes Played', fontsize=20)
